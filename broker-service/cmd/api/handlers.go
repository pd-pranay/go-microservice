...
func (app *App) logItem(w http.ResponseWriter, entry LogPayload) {
	...
	response, err := app.sendPOSTRequest(entry, logServiceURL)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *App) authenticate(w http.ResponseWriter, a AuthPayload) {
	...
	response, err := app.sendPOSTRequest(a, "http://auth-service/users/auth")
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *App) sendPOSTRequest(a any, url string) (*http.Response, error) {
	...
	response, err := app.newClient.Client.Do(request)
	if err != nil {
		return nil, err
	}
